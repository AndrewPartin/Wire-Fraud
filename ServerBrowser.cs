...
public partial class ServerBrowser : Control
{
	...
	public override void _Process(double delta)
	{
		if (listener.GetAvailablePacketCount() > 0) {
			...
			Node currentNode = GetNode<VBoxContainer>("Panel/Servers").GetChildren().Where(x => x.Name == info.Name).FirstOrDefault();

			if (currentNode != null) {
				currentNode.GetNode<Label>("Player Count").Text = info.PlayerCount.ToString();
				currentNode.GetNode<Label>("IP").Text = serverIP; // doesnt need to update every time, but not sent with first TODO: fix lol
				return;
			}
			
			LanServerInfoLine serverInfo = ServerInfo.Instantiate<LanServerInfoLine>();
			serverInfo.Name = info.Name;
			serverInfo.GetNode<Label>("Server Name").Text = serverInfo.Name;
			serverInfo.GetNode<Label>("IP").Text = serverIP;
			serverInfo.GetNode<Label>("Player Count").Text = info.PlayerCount.ToString();
			GetNode<VBoxContainer>("Panel/Servers").AddChild(serverInfo);
